...
//
// Which codes this system actually collects in is not decided here: a list in
// the domain would make adding a currency a deploy of this service, and the
// answer belongs to whoever runs the shop. What is checked is the shape the
// schema relies on, so CHECK (amount_currency ~ '^[A-Z]{3}$') holds for every
// row this package can produce.
...
//
// Copied from the order service rather than shared with it, which is the trade
// this repo makes everywhere a domain concept appears twice: a shared Money
// would be one type two services could not change independently, and the day
// one of them needs a rounding rule the other must not have, the shared version
// is the problem rather than the saving.
type Money struct {
	amountMinor int64
...
// constraint on it would forbid those everywhere it is reused. Nothing this
// service collects is negative, so this is where it is rejected.
...
// that had to call NewMoney would fail to load the very attempts somebody needs
...
// Equal reports whether two amounts are the same money. Currency is part of the
// value, so THB 100 and USD 100 are not equal and never compare as such.
