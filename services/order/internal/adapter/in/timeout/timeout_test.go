...
// sweep is a scripted in.SagaTimeout. It answers with what the test asked for
// and publishes the limit it was called with, so the assertions are about when
// the loop decided to sweep rather than about a counter the two goroutines
...
func (s *sweep) ExpireStaleOrders(_ context.Context, limit int) (int, error) {
	call := int(s.n.Add(1))
	...
	// channel nobody drains — a blocking send there would hold the worker
	// inside this call, where it never reaches the context check that stops it,
	// and shutdown would time out rather than the assertion failing.
	select {
	case s.calls <- limit:
	default:
	}
...
// start runs the worker and returns the function that stops it and checks how
// it stopped.
func start(t *testing.T, s *sweep, cfg timeout.Config) func() {
	t.Helper()

	w, err := timeout.New(s, cfg, timeout.WithRegisterer(prometheus.NewRegistry()))
	if err != nil {
...
func waitForSweep(t *testing.T, s *sweep) int {
	...
	case limit := <-s.calls:
		return limit
	case <-time.After(5 * time.Second):
		t.Fatal("the worker did not sweep")
...
func TestRunSweepsAgainImmediatelyOnAFullBatch(t *testing.T) {
	...
	stop := start(t, s, timeout.Config{Interval: time.Hour, BatchSize: 2, MaxBackoff: time.Hour})
	defer stop()
...
func TestRunWaitsAfterAShortBatch(t *testing.T) {
	...
	// interval away — otherwise an idle worker would spin on the database.
	s := newSweep(func(int) (int, error) { return 1, nil })

	stop := start(t, s, timeout.Config{Interval: time.Hour, BatchSize: 10, MaxBackoff: time.Hour})
	defer stop()
	...
	select {
	case <-s.calls:
		t.Error("the worker swept again without waiting out its interval")
	case <-time.After(200 * time.Millisecond):
	}
}

func TestRunKeepsGoingAfterAFailure(t *testing.T) {
	// A failed sweep is rolled back, so the orders are still stale and still
	// there. Giving up would strand every order behind the one that failed, and
	// a process that exited would be restarted into the same failure.
...
func TestNewRejectsAConfigThatCannotWork(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "no interval", cfg: timeout.Config{BatchSize: 10, MaxBackoff: time.Minute}},
		{name: "no batch", cfg: timeout.Config{Interval: time.Second, MaxBackoff: time.Minute}},
		// A ceiling below the floor would make the backoff shrink the wait
		// instead of growing it.
		{name: "backoff under the interval", cfg: timeout.Config{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := timeout.New(newSweep(ok), tt.cfg, timeout.WithRegisterer(prometheus.NewRegistry()))
			if err == nil {
	...
	t.Run("no sweep to drive", func(t *testing.T) {
		cfg := timeout.Config{Interval: time.Second, BatchSize: 10, MaxBackoff: time.Minute}

		if _, err := timeout.New(nil, cfg, timeout.WithRegisterer(prometheus.NewRegistry())); err == nil {
			t.Fatal("New() error = nil, want a configuration error")
		}
	})
}
