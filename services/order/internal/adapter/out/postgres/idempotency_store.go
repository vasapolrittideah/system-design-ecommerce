...
// Claim takes the key for orderID, reporting whether this call took it.
//
// The INSERT is what does the deciding, and it has to run in the transaction
// that writes the order: a concurrent submit blocks on the primary key until
...
func (s *IdempotencyStore) Claim(
	...
	order, err := parseID(orderID.String(), "order id")
	if err != nil {
		return out.Claim{}, false, err
	}
