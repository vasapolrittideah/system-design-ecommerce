...
deploy/k8s/
  infra/                 # the dependencies every cluster shares, described once
    ...
    namespace/            # the namespace an environment runs in, named by its overlay
  ...
  overlays/{local,staging,prod}/namespace/    # which namespace this environment is
  apps/{staging,prod}/   # what Argo reconciles each against — one file per overlay
```
...
| Risk | Mitigation |
| --- | --- |
| Duplicate client submit | `Idempotency-Key` on the request → `(key, request_hash) → aggregate id` in the orchestrator's own database, kept 24h |
...
| Saga stuck mid-flight | timeout worker that sweeps aggregates left in a waiting state and forces compensation through the domain |
...
**That first row is a table in the orchestrator's own database, not a key in Redis.** Claiming the key and persisting the aggregate have to be one act: a claim that commits against a write that rolls back answers the client's retry with a replayed response for an order that does not exist, and Redis cannot join the transaction that would prevent it. In Postgres it is the same `INSERT ... ON CONFLICT DO NOTHING` a consumer makes in `processed_events`, on the transaction already writing the aggregate. It stores an answer rather than only a claim, so it needs the one thing the claim does not: the request hash, which tells a retry from a key reused for different content. What it stores is the aggregate's id and not a serialized response — replaying means loading the aggregate, so there is one copy of it in the database rather than two that can disagree.

**It carries no in-flight state, and wanting one is a sign the claim has been put in the wrong transaction.** Committed with the aggregate, the row is invisible until it is also final: a second submit arriving meanwhile blocks on the primary key until the first transaction ends, and then either reads the answer or takes the key that transaction's rollback released. Postgres does the waiting a state column would otherwise have to describe, which is the same reason `inbox.Claim` needs no state either. A claim that commits *before* the work does need one — and needs a sweep for the rows whose work then died, which is the cost of moving it there.
...
**That last row is a sweep over the aggregate's own table, not a `saga_instance` table beside it.** An order *is* its saga: it has exactly one waiting state, and `created_at` is the deadline. A second table describing the same thing would be a second source of truth to keep in step, and the failure it invites — a saga row that disagrees with the aggregate — is worse than the one it prevents. What earns a `saga_instance` table is a flow with several waiting states, where "which step is this stuck on" stops being answerable from the aggregate's own status; until then the sweep is `WHERE status = '<waiting>' AND created_at <= $1 ... FOR UPDATE SKIP LOCKED`, and the transition it calls is a method on the aggregate that re-checks the deadline itself. The query and the domain both judging is what stops a predicate that drifted from cancelling orders somebody is still paying for.

**A step failing is not the same as the flow ending, and only the orchestrator may decide it is.** A declined card leaves the order open: declines are ordinary and mostly transient, every storefront worth copying lets the customer try another card, and payment's schema is built for it — a failed attempt drops out of the partial unique index that allows one unresolved payment per order, precisely so the next may begin. So `PaymentFailed` moves nothing. Running out of time is the only thing that ends an unpaid order, which is what makes the timeout worker load-bearing rather than a backstop: without it nothing ever cancels one.

**The timeout has to fire before the resource it is compensating expires, and this is the first coupling here that crosses services.** `ORDER_PAYMENT_WINDOW` (10m) must stay below `INVENTORY_RESERVATION_TTL` (15m) by more than one sweep interval. Too long and the hold expires first: the customer pays, the order is marked paid, and the commit that turns the hold into a sale is refused because the reservation is gone — money taken for stock somebody else can now buy, landing in a DLQ for a person to sort out. Too short is merely rude. Neither side reports the other, and the margin has to cover the sweep interval, the outbox relay's poll, and both consumers.
...
**A provider webhook is north-south traffic and arrives here like everything else, and the BFF must not read it.** Verifying the signature needs the provider's secret, and knowing what the body means is the owning service's business; a BFF has neither, so it forwards the raw bytes and the signature header over gRPC and interprets nothing. The secret then lives in the service rather than in the process most exposed to the outside.

It sits outside the authenticated group, as the auth and browse routes do, but for a different reason: those are open because the caller may not have an account yet, and this one because the caller is not a person. What stands in for identity is the signature, which proves who wrote the bytes rather than who relayed them — a stronger claim than a forwarded user id would have been.

The body must be passed through **byte for byte**. The signature is computed over exactly those bytes, so a hop that decoded the JSON and re-encoded it forwards a body that no longer verifies — a different key order is enough — and it fails as a rejected webhook with nothing pointing at the hop that broke it. It travels as `bytes` on the request for that reason.
...
- **Logs reach Loki by being tailed, never by being pushed.** A process writes stdout and knows nothing else; the kubelet writes that to a file on the node, and Grafana Alloy — one per node, `deploy/k8s/infra/alloy` — tails the file and pushes to Loki. A process that shipped its own logs would buffer them in its heap and lose them in a crash, which is the one moment they matter. Two rules hold the arrangement together, and both fail quietly:
  - **A label is a stream, so the label set stays small**: `service`, `pod`, `container`, `stream`, `level`. `trace_id` is deliberately *not* a label — it is the highest-cardinality field in the system, and indexing it is how a log backend runs out of memory. It is found by filtering the line, which is what Grafana's `tracesToLogsV2` does from a span and what `derivedFields` reads back off the raw line to link into Jaeger. That regex names `trace_id` literally: renaming the field in `pkg/logger` breaks the link with nothing reporting it.
  - **`service` means the same thing on a series, a span, and a log stream.** That one shared name is the whole of what lets a dashboard move between the three, and it is why Alloy relabels `app.kubernetes.io/name` to `service` exactly as the scrape config does.
...
**A third party is stood in for by a fake in this repo, never by its own sandbox.** A real provider's test mode will decline a card, which is the easiest case; it will not, on demand, time out with the charge already made, deliver one webhook twice, or deliver the webhook before the response to the call that caused it — and those are the cases the write-before-you-call ordering, the inbox, and the row locks in settling exist for. `services/payment/internal/fakeprovider` speaks the same API the adapter does and can be told to do all of them; `cmd/fakeprovider` runs it in the cluster and the adapter's tests run it in process, over real HTTP with a real HMAC, because the failures that package has to get right are transport failures.

Which behaviour it takes is **the last two digits of the amount**, so a test — or a person clicking through the storefront — chooses one by choosing what to buy rather than by reaching past the service under test to configure the thing behind it. The table is in `cmd/fakeprovider`.

Which provider a deployment talks to is a value and not a branch: `PAYMENT_PROVIDER_BASE_URL` is the whole of the difference, the fake is a kustomize component only `local` includes, and signing with a real one means a sibling adapter against their API rather than an `if` anywhere. The fake ships in the service's own image picking a different binary, so it cannot be built from a commit the service was not — which is the one way a stand-in starts lying about the API it stands in for.
...
- **The numbers are not a capacity result.** Against `local`, generator, cluster, and every database share one laptop, and the local overlays request 10m of CPU per pod so that the HPA is reachable at all. What transfers is the shape: which limit is met first, which error the system answers with, and whether scaling out changed it.
- **`make load OVERLAY=staging` is the run that says something about prod**, because staging pulls the images CI published, decrypts the credentials the same controller holds, and answers on a tunnel — none of which a laptop has. Two costs come with it, and neither is avoidable by tuning. Requests cross the internet to a Cloudflare edge and back down a tunnel, so every number carries that round trip and the generator's own uplink saturates before the cluster does; a run whose subject is the cluster belongs on the k3s host, with `BASE_URL` pointed at a port-forward of Kong. And staging shares a host and a kernel with prod, so load here is CPU prod does not get — watch prod's dashboards during one.
- **`make load OVERLAY=prod` is refused** unless `CONFIRM=prod` is passed. The limits Kong holds are what stands between one client and everybody else's service, and this tool's first act is to raise them.
...
make up [OVERLAY=prod]  # namespace, cluster singletons, then that cluster's shared infra
make deploy SVC=x       # build image, run the migration Job, roll out
make stack              # deploy every service, BFFs last, then smoke
make stack OVERLAY=staging BUILD=0  # same, pulling the images CI published
make keys OVERLAY=staging   # ES256 keypair, public half to every verifier
make seal OVERLAY=staging   # encrypt that overlay's credentials for this cluster
make argocd             # install the controller that reconciles staging and prod
...
CI applies `local` to a k3d cluster it creates and throws away, which is what keeps the manifests from being files rather than an environment. It is deliberately not `staging`: that overlay pulls published images by SHA, which do not exist for an unmerged commit, and reads credentials sealed against the k3s host, which a cluster a minute old holds no key for. **What CI therefore does not check is base's own values** — its resource requests, its HPA bounds, and everything in `base/*/config.env`. A bad default there passes the pull request and is caught minutes after merge, when staging syncs and its smoke test runs. That is the trade for naming overlays after places rather than after jobs, and staging following trunk automatically is what makes it survivable.

`staging` and `prod` are the two deployed environments, and they **share one k3s host, separated by a namespace**: `ecommerce-staging` and `ecommerce`. That is the entire structural difference between them — same images from the same registry, same sealed-secrets controller, same Argo, one root Application and one AppProject each. Everything else follows from it: a SealedSecret sealed for one namespace cannot be read in the other, each runs its own Postgres instances and its own observability stack, and each dials out through a Cloudflare tunnel of its own.

The one behavioural difference is **who moves the image tag**. CI bumps staging's on every push to trunk, so staging is trunk's tip and needs nobody to remember it; prod's is bumped by a person running `Deploy prod`, so a release is a promotion of something that has already been running. An environment somebody has to remember to deploy falls months behind and then proves nothing about the release standing behind it — which is exactly what a staging environment is for.

Two things follow from sharing a host, and neither is free. A load test on staging is CPU prod does not get, so it is a prod incident waiting for a coincidence — watch prod while running one. And a cluster-scoped object cannot be deployed per environment: **Reloader moved to `kube-system`**, installed by `make up` beside the sealed-secrets controller, because two environments deploying its ClusterRole and ClusterRoleBinding would be two Argo Applications rewriting one object between them. After that move, neither Argo project needs any cluster-scoped permission at all.
...
Setting up a tunnel is four commands on the operator's machine, and the runbook is in `overlays/<env>/infra/cloudflared.yaml` beside the config it produces — there are two of them, one per durable environment: `cloudflared tunnel login`, `tunnel create`, copy the credentials JSON into that directory (gitignored, sealed by `make seal OVERLAY=prod`), then `tunnel route dns` to point `api.vasapol.dev` at it. That last one is the step whose absence looks exactly like a healthy deploy — every pod Ready, no request ever arriving.
...
- **A SealedSecret can only be read by the cluster it was sealed against**, and encryption is scoped to namespace and name together, so it can be neither moved nor renamed. Sealing on a laptop and applying to a server produces a resource the controller there cannot decrypt, and that is the one failure among these that does not resolve itself.
- **The controller lives in `kube-system`, not in `ecommerce`**, which is why it is applied on its own rather than through the infra kustomization. It generates its sealing key into a Secret beside itself, and `make clean-volumes` deletes the `ecommerce` namespace — a key living there would make every committed SealedSecret permanently undecryptable, from a target whose purpose is throwing away local data. Back the key up; the command is in that kustomization.
- **The readable material stays on the operator's machine, gitignored**, because re-sealing after a controller is reinstalled needs it. Losing it means rotating every credential rather than re-encrypting them — and rotating a database password after `initdb` has run does not change the database's password, only what the service tries to log in with.

**Images are published to ghcr on every trunk commit, tagged with that commit's SHA and nothing else.** `ghcr.io/<owner>/<repo>/<service>:<sha>`, plus `<service>-migrate` where the service owns a database. A tag that follows a branch is one somebody deploys by accident, and it is also the tag a GitOps controller cannot see change — the reference has to name one build forever, which is what makes a rollback a reference change rather than a rebuild. `overlays/staging` and `overlays/prod` are what point at them, one `newTag` line per image, read back off the file to know what is running. staging's are rewritten by the `promote-staging` job in CI on every push to trunk; prod's by the `Deploy prod` workflow when a person dispatches it. `local` keeps building and importing, because the cluster running it is on the machine doing the deploy.
...
**staging and prod are both reconciled from `trunk` by Argo CD, and a commit is what a deploy to either now is.** `deploy/k8s/infra/argocd` holds the controller, vendored and pinned the way sealed-secrets is; `deploy/k8s/apps/<env>` holds one Application per overlay and a root that manages the directory they sit in, so adding a service is adding a file and removing one is removing a file. `make argocd` installs it, `kubectl apply -f deploy/k8s/apps/<env>/root.yaml` starts each, and after that a push is the deploy and a `git revert` is the rollback.

**Two roots and two AppProjects rather than one of each**, because the two environments have to be startable and stoppable independently — taking staging down for an afternoon is one `kubectl delete` of `root-staging`, and prod's reconcile never notices. Each project names one namespace as its only destination, so an Application pointed at the wrong overlay by a bad merge is refused rather than applied: the namespace is the boundary between the two environments, and this is what stops an Argo object from stepping over it.

What it deliberately does not do is decide which image *prod* runs: that `newTag` is bumped by a person dispatching a workflow, and an updater that writes tags into git from inside the cluster would need a credential that can push to trunk living there. staging's tags are written by CI instead, which is a different trade — a workflow that already has the token, pointing an environment nobody has to remember at the commit it just published. What Argo buys is that deploying no longer depends on where an operator's `kubectl` happens to point — a failure this repository has already had once, when a `make seal OVERLAY=prod` sealed every credential against a laptop's cluster.

**Bringing a durable environment up the first time** is the one sequence where order matters, because three of its steps fail silently out of order. With `kubectl` pointed at the k3s host:

```text
make up OVERLAY=staging          # namespace + the two cluster singletons, then
                                 # FAILS on the infra overlay: nothing is sealed yet
make keys OVERLAY=staging        # ES256 pair; public half copied to bff-web
cloudflared tunnel create ecommerce-staging   # then copy its JSON into the overlay
make seal OVERLAY=staging        # DB passwords + tunnel credentials, sealed for THIS cluster
make up OVERLAY=staging          # now the overlay renders, and the infra lands
cloudflared tunnel route dns ecommerce-staging staging-api.vasapol.dev
git add deploy/k8s/overlays/staging && git commit && git push   # the sealed files
kubectl apply -f deploy/k8s/apps/staging/root.yaml
```

The first `make up` failing is the expected path rather than a mistake, and it is why the sequence is written out: `make seal` cannot run before the controller that holds the key exists, and `make up` is what installs it. It applies sealed-secrets and reloader before the overlay for exactly that reason.

Four things here fail in ways that do not announce themselves. `make seal` run from a laptop seals against the laptop's cluster and produces resources the host can never decrypt — the one failure in this list that does not resolve itself on a retry. `route dns` omitted looks exactly like a healthy deploy: every pod Ready, no request ever arriving. The sealed files left uncommitted leave Argo unable to render the directory at all, which is the same state the `promote-staging` job checks for before it tries to bump anything. And the whole sequence run against the wrong `kubectl` context puts a second environment inside whatever cluster that was.
...
- **The Applications run under an AppProject that is an allow-list**, rather than Argo's `default`, which permits any repository, any cluster, and any kind. Each names one repository, one namespace, and — since Reloader moved to `kube-system` — no cluster-scoped kind at all. Prometheus and Alloy need none: their discovery is namespace-scoped and so is their RBAC. A Namespace, a CRD, a webhook, or a PersistentVolume appearing in a render is refused. Same instinct as the NetworkPolicies: the rule sits beside what it constrains, and a change that steps outside it fails rather than working.
...
- **The migration Job is a `Sync` hook at wave 1, with `BeforeHookCreation`**, which tells Argo what `make deploy` does by hand — delete the previous Job because a Job is immutable, run it, wait for it, then roll out. The annotations are inert where Argo is not watching, so the two paths describe one ordering rather than competing for it. The waves are 0 for config, secrets and the database, 1 for the migration, 2 for the Deployments, 3 for the HPA, and Argo waits for each to report healthy before starting the next.
- **It is deliberately not `PreSync`, and that distinction cost staging its first sync.** A PreSync hook runs before Argo applies *any* manifest, so on a namespace that does not already hold them the Job finds no ConfigMap, no Secret and no database, and waits on resources whose creation it is itself blocking. prod survived only because everything it needs predates Argo — which is the same thing as saying prod could not have been rebuilt from an empty namespace. The subtler half applies to a running cluster too: a PreSync hook reads the *previous* revision's ConfigMap and Secret, so one commit changing both a database host and an image tag would migrate against the old host and then start pods on the new.
...
**Anything a reconciled overlay references has to be in git, and `.gitignore` is where that breaks.** `overlays/prod/bff-web` — and now `overlays/staging/bff-web` — mounts the ES256 verification key through a `configMapGenerator` while `*.pem` is ignored, so that overlay could not be rendered from a clean checkout from the day it was written — and nothing reported it, because every path that had ever rendered it was also the machine that ran `make keys`. Argo is the first thing here that reads this repository without having written to it, and it failed on the first sync. The public half is committed now, by a negation narrow enough to name one file; it verifies a signature and cannot make one. The rule it leaves behind is the general one: staging and prod are rendered somewhere else, so a file that only an operator has is a file neither of them has.
...
**The namespace denies ingress by default, and each workload carries the allow-list of who may reach it.** `deploy/k8s/infra/namespace/` holds the deny — with the Namespace rather than in the infra kustomization, because `make down` removes that kustomization while the services it was protecting keep running. It is a base rather than a file because two environments share a cluster: `overlays/<env>/namespace` says which namespace this is, and kustomize's namespace transformer renames the Namespace object itself, so the name is written once. Everything else sits beside the thing it protects: a service's in `base/<name>/networkpolicy.yaml`, a database's in the component that declares the database, an infra component's next to its Deployment.
...
One Deployment per service with HPA on CPU or consumer lag (KEDA) — plus one more for each background loop the service runs, which is where `inventory`'s reservation reaper and `order`'s saga timeout worker sit. A loop that ran as a goroutine inside the server would be started once per replica, so the work it does would be multiplied by whatever the autoscaler decided on. Migrations run as Jobs/init containers. gRPC needs a headless service with client-side load balancing — an L4 load balancer pins a single connection. Config comes from env (12-factor); secrets from External Secrets / Sealed Secrets.

**A Kafka consumer is not one of those loops, and several belong in one process.** A consumer group hands each partition to exactly one member, so a second replica divides the work rather than repeating it — which is the whole of what makes a timer different, since a timer has no such thing and two of them sweep the same rows. `order`'s worker accordingly runs both of its subscriptions, one reading the payment service's events and one reading its own, while its timeout worker is a Deployment of its own. Splitting the subscriptions would buy nothing and cost a second workload; folding the timer in with them would multiply its row locks by the replica count.

Two subscriptions in one process still take two consumer groups. The group is half the key in `processed_events`, so a shared id lets a delivery on one topic mask a delivery on the other that happened to carry the same event id — and it collapses two lag series into one number nobody can attribute.
...
**Reloader itself lives in `kube-system`, not in the namespace it serves**, and `make up` installs it there the way it installs the sealed-secrets controller. It is a cluster singleton: its RBAC is a ClusterRole and a ClusterRoleBinding, which are one object each for the whole cluster, so a copy per environment would be two Argo Applications flapping one binding between two ServiceAccounts — and a reloader intermittently unable to read what it watches. One instance watching every namespace is what that ClusterRole was always for.
